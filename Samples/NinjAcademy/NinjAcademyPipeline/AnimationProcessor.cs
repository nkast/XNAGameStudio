    ...
    public class AnimationProcessor : ContentProcessor<XDocument, AnimationStore>
    {
        public override AnimationStore Process(XDocument input, ContentProcessorContext context)
        {
            ...
            foreach (XElement animationDefinition in definitions)
            {
                ...
                visualCenter.X = float.Parse(animationDefinition.Attribute("VisualCenterX").Value, CultureInfo.InvariantCulture);
                visualCenter.Y = float.Parse(animationDefinition.Attribute("VisualCenterY").Value, CultureInfo.InvariantCulture);
                ...
                animation.SetFrameInterval(TimeSpan.FromMilliseconds(
                    double.Parse(animationDefinition.Attribute("FrameMillisecondInterval").Value, CultureInfo.InvariantCulture)));
