...
namespace NinjAcademy.Pipeline
{
    ...
    public class ConfigurationProcessor : ContentProcessor<XDocument, GameConfiguration>
    {
        ...
        private GamePhase ParsePhaseElement(XElement phaseElement)
        {
            ...
            // Get phase duration
            result.Duration = TimeSpan.FromSeconds(double.Parse(phaseElement.Attribute("Duration").Value, CultureInfo.InvariantCulture));

            // Get gold target probability
            result.GoldTargetProbablity = 
                double.Parse(phaseElement.Element("Targets").Attribute("GoldProbability").Value, CultureInfo.InvariantCulture);
            ...
            for (index = 0; index < TargetAmount; index++)
            {
                result.TargetAppearanceIntervals[index] =
                    TimeSpan.FromSeconds(double.Parse(targetElements.ElementAt(index).Attribute("Interval").Value, CultureInfo.InvariantCulture));
                result.TargetAppearanceProbabilities[index] =
                    double.Parse(targetElements.ElementAt(index).Attribute("Probability").Value, CultureInfo.InvariantCulture);
            ...
            result.BambooAppearanceInterval =
                TimeSpan.FromSeconds(double.Parse(bambooElement.Attribute("Interval").Value, CultureInfo.InvariantCulture));
            result.BambooAppearanceProbablity = double.Parse(bambooElement.Attribute("Probability").Value, CultureInfo.InvariantCulture);
            ...
            result.DynamiteAppearanceInterval =
                TimeSpan.FromSeconds(double.Parse(dynamiteElement.Attribute("Interval").Value, CultureInfo.InvariantCulture));
            result.DynamiteAppearanceProbablity = double.Parse(dynamiteElement.Attribute("Probability").Value, CultureInfo.InvariantCulture);
            ...
            {
                double probability = double.Parse(amountProbabilityElement.Attribute("Probability").Value, CultureInfo.InvariantCulture);
